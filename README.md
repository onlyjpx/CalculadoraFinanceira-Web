...
Abra um terminal na pasta `backend/` e execute conforme seu setup:

- Se você usa IntelliJ IDEA:
  - Abra a pasta `backend/`
  - Rode a classe `BackendApplication` (arquivo `backend/src/main/kotlin/com/calculadora/backend/BackendApplication.kt`)

- Via Maven (se existir `mvnw`/Maven no seu ambiente):
  - `mvn spring-boot:run`
